<div class="sticky-header-wrap sticky-header py-1 py-sm-2 py-lg-1">
    <div class="container">
        <div class="row align-items-center">
            <div class="col-5 col-md-3">
                <div class="logo">
                    <a href="index.php"><img src="assets/img/logo.png" alt="logo" style="width: 5rem;"/></a>
                </div>
            </div>
            <div class="col-7 col-md-9 text-right">
                <nav class="main-menu menu-sticky1 onepage-nav d-none d-lg-block link-inherit">
                    <ul>
                        <li><a href="index.php">Inicio</a></li>
                        <li><a href="#about">Nosotros</a></li>
                        <li><a href="#food-menu">Menu</a></li>
                        <li><a href="#gallery">Galería</a></li>
                        <li><a href="#contact">Contacto</a></li>
                    </ul>
                </nav>
                <button class="vs-menu-toggle text-theme border-theme d-inline-block d-lg-none">
                    <i class="far fa-bars"></i>
                </button>
            </div>
        </div>
    </div>
</div>
<div class="vs-menu-wrapper">
    <div class="vs-menu-area">
        <button class="vs-menu-toggle text-theme">
            <i class="fal fa-times"></i>
        </button>
        <div class="mobile-logo">
            <a href="index.php"><img style="width: 5rem;" src="assets/img/logo.png" alt="logo" /></a>
        </div>
        <div class="vs-mobile-menu onepage-nav link-inherit"></div>
    </div>
</div>
<header class="header-wrapper header-layout2 bg-dark dark-style py-2 py-lg-0 px-xl-115">
    <div class="container-fluid">
        <div class="row align-items-center">
            <div class="col-6 col-md-3 col-lg-2 col-xl-2">
                <div class="header-logo">
                    <a href="index.php"><img src="assets/img/logo-bco.png" style="width: 5rem;" alt="Logo" /></a>
                </div>
            </div>
            <div class="col-6 col-md-9 col-lg-7 col-xl-5">
                <nav class="main-menu menu-style1 onepage-nav link-inherit text-right text-xl-left mobile-menu-active">
                    <ul>
                        <li><a href="index.php">Inicio</a></li>
                        <li><a href="#about">Nosotros</a></li>
                        <li><a href="#food-menu">Menu</a></li>
                        <li><a href="#gallery">Galería</a></li>
                        <li><a href="#contact">Contacto</a></li>
                    </ul>
                </nav>
                <button type="button" class="vs-menu-toggle ml-auto d-block text-theme border-theme d-lg-none">
                    <i class="far fa-bars"></i>
                </button>
            </div>
            <div class="col-lg-3 col-xl-5">
                <div class="header-right d-none d-lg-flex align-items-center justify-content-end">
                    <div class="contact-info media align-items-center d-none d-xl-flex">
                        <div class="media-icon mr-15 pl-30" data-overlay="theme" data-opacity="1">
                            <i class="fal fa-clock text-theme fa-2x"></i>
                        </div>
                        <div class="media-body">
                            <span class="d-block mb-1 text-white-light">Llama para ordenar</span>
                            <p class="mb-0 h4 text-font1">
                                <a href="tel:+526246882481">624 688 2481</a>
                            </p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</header>